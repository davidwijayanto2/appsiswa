...
import { addGuruAPI, getAllGuruAPI, getGuruAPI, editGuruAPI, deleteGuruAPI } from '../repos/http_requests';
...
export default function guru() {
    ...
    const [namaGuruInput, setNamaGuruInput] = useState('');
    ...
    const emptyRows =
        page > 0 ? Math.max(0, (1 + page) * rowsPerPage - listGuru.length) : 1;
    ...
    const openEditForm = async (event, id_guru) => {
        event.preventDefault();
        setEdit(true);
        const response = await getGuruAPI(id_guru);
        console.log(response);
        if (response.status == 200) {
            const resJson = await response.json();
            const data = resJson.data;
            setNamaGuruInput(data.namaGuru);
    ...
    const openDeleteForm = async (event, id_guru, nama_guru) => {
        event.preventDefault();
        setIdGuru(id_guru);
        setNamaGuru(nama_guru);
        handleOpenDelete();
    }
    ...
    async function submitGuru() {
        ...
        if (validationField(errors, setErrors, schema, body)) {
            let response;
            if (!isEdit) {
                response = await addGuruAPI(body);
            } else {
                response = await editGuruAPI(idGuru, body);
            }
            if (response.status == 200) {
                fetchDataGuru();
                handleCloseAdd();
    ...
    async function deleteGuru() {
        const response = await deleteGuruAPI(idGuru);
        if (response.status == 200) {
        fetchDataGuru();
            handleCloseDelete();
            const data = await response.json();                        
    ...
    return (
        <Layout title="Guru" subtitle="Guru">
            <div className="h-full container md:px-14">
                <SubtitleAdmin subtitle="Master Guru" />
                <div className="flex justify-end w-full my-4 px-0 md:px-4 xl:px-2">
                    ...
                    >
                        ...
                        <p className="capitalize font-semibold text-xl">Tambah Guru</p>
                    </button>
                    <OpenDialog title={!isEdit ? "Tambah Guru" : "Ubah Guru"} handleClose={handleCloseAdd} isOpen={isOpenAdd} fullWidthStatus={true} maxValue="sm">
                        <div className="flex flex-col px-1 w-full">
                            <label htmlFor="nama_guru" className="text-xl font-medium flex capitalize">Nama Guru<p className="text-red-500 mx-1">*</p></label>
                            <input
                                type="text"
                                ...
                                value={namaGuruInput}
                                maxLength = "255"
                                onChange={(e) => setNamaGuruInput(e.target.value)}
                                onKeyDownCapture={handleKeyDown}
                                autoFocus
                            />
                            <span style={{ color: "red" }}>
                                {errors.nama_guru ? errors.nama_guru.replaceAll("_"," ") : ""}
                            </span>
                        </div>        
                        <div className="flex flex-col px-1 w-full">
                    ...
                    <OpenDialog title="Hapus Guru" handleClose={handleCloseDelete} isOpen={isOpenDelete} fullWidthStatus={true} maxValue="sm">
                        <div className='flex justify-center'>
                            <span className='text-xl text-center'>Apakah Anda yakin akan menghapus guru "{namaGuru}"?</span>
                        </div>
                    ...
                    {isLoading ? <div>Loading.....</div> :
                        <Paper sx={{ width: '100%', overflow: 'hidden' }} className="bg-transparent">
                            <TableContainer>
                                <Table>
                                    <EnhancedTableHead
                                        ...
                                        rowCount={listGuru ? listGuru.length : 0}
                                    />
                                    <TableBody>
                                        {                                                                                     
                                            stableSort(listGuru ? listGuru : [], getComparator(order, orderBy))
                                                ...
                                                .map((row, index) => {
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            <TableCell className="text-xl font-medium">{row.namaGuru}</TableCell>
                                                            ...
                                                            <TableCell>
                                                                <div className="flex flex-row justify-end">
                                                                    <button
                                                                        type="button"
                                                                        onClick={event => openEditForm(event, row.idGuru)}>
                                                                    ...
                                                                    <button
                                                                        type="button"
                                                                        onClick={event => openDeleteForm(event, row.idGuru, row.namaGuru)}>
                            ...
                            <TablePagination
                                ...
                                component="div"
                                count={listGuru ? listGuru.length : 0}
